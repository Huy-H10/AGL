// dongBoDuLieuNguoiDung.js - Đồng bộ dữ liệu người dùng
// FIX: Hook addParticipant / handleCheckin sau khi module đã expose chúng
...
// ── 2. ĐỒNG BỘ participants → userDatabase ────────────
function syncUserDatabase() {
  ...
  participants.forEach(p => {
    ...
    if (existing) {
      existing.hoten = p.name  || p.hoTen || existing.hoten;
      existing.email = p.email || existing.email;
      existing.sdt   = p.sdt   || existing.sdt || "";

      const checkedEvents = participants.filter(x => x.mssv === p.mssv && x.checkedIn).map(x => x.event);
      existing.trangThai = checkedEvents.length > 0 ? "Đã check-in" : "Chưa check-in";

      const allEvents = [...new Set(participants.filter(x => x.mssv === p.mssv).map(x => x.event))];
...
function syncCheckinStatus(mssv) {
  ...
  const checkedEvents = participants.filter(p => p.mssv === mssv && p.checkedIn).map(p => p.event);
  record.trangThai = checkedEvents.length > 0 ? "Đã check-in" : "Chưa check-in";
  record.updatedAt = new Date().toISOString();
  saveUserDB(db);
}

// ── 4. INJECT TRANG DATABASE VÀO DOM ─────────────────
    ...
    page.innerHTML = `
      <div class="card" style="max-width:950px;">
        ...
        <div style="display:flex;gap:10px;margin-bottom:15px;flex-wrap:wrap;">
          <input id="dbSearch" placeholder="🔍 Tìm theo tên, MSSV, email..."
            style="flex:1;min-width:200px;padding:10px;border:2px solid #ffe0ea;border-radius:10px;"
            oninput="filterUserDB()">
          <select id="dbFilterStatus" style="padding:10px;border:2px solid #ffe0ea;border-radius:10px;" onchange="filterUserDB()">
            <option value="">-- Tất cả trạng thái --</option>
            <option value="Đã check-in">✅ Đã check-in</option>
            <option value="Chưa check-in">⏳ Chưa check-in</option>
          </select>
          <button onclick="syncUserDatabase(); renderUserDB();" style="padding:10px 16px;background:#ff6a88;color:#fff;border:none;border-radius:10px;cursor:pointer;font-weight:700;">🔄 Đồng bộ</button>
          <button onclick="exportUserDB()" style="padding:10px 16px;background:#2ecc71;color:#fff;border:none;border-radius:10px;cursor:pointer;font-weight:700;">📥 Xuất CSV</button>
        </div>
        <div style="overflow-x:auto;">
          <table>
            <thead>
              <tr><th>Họ tên</th><th>MSSV</th><th>Email</th><th>SĐT</th><th>Trạng thái</th><th>Sự kiện tham gia</th><th>Cập nhật</th></tr>
            </thead>
            <tbody id="userDBTable"></tbody>
          </table>
        </div>
        <p id="dbEmpty" style="text-align:center;color:#aaa;padding:20px;display:none;">Chưa có dữ liệu. Nhấn 🔄 Đồng bộ để tải.</p>
      </div>`;
...
function renderUserDB(data) {
  ...
  tbody.innerHTML = "";
  if (db.length === 0) { if (empty) empty.style.display = "block"; return; }
  ...
  db.forEach(u => {
    ...
    tbody.innerHTML += `
      <tr>
        ...
        <td>${u.sdt||"—"}</td>
        <td>${badge}</td>
        <td style="font-size:12px;color:#666;">${(u.suKienThamGia||[]).join(", ")||"—"}</td>
        <td style="font-size:11px;color:#aaa;">${u.updatedAt ? new Date(u.updatedAt).toLocaleString("vi-VN") : "—"}</td>
      </tr>`;
  });
}
...
function filterUserDB() {
  ...
  let db = getUserDB();
  if (keyword) db = db.filter(u => (u.hoten||"").toLowerCase().includes(keyword) || (u.mssv||"").toLowerCase().includes(keyword) || (u.email||"").toLowerCase().includes(keyword));
  if (status)  db = db.filter(u => u.trangThai === status);
  renderUserDB(db);
}
...
function exportUserDB() {
  const db = getUserDB();
  if (db.length === 0) {
    if (typeof window.showToast === "function") window.showToast("Không có dữ liệu để xuất!", "error");
    return;
  }
  let csv = "Họ tên;MSSV;Email;SĐT;Trạng thái;Sự kiện tham gia;Cập nhật\n";
  db.forEach(u => {
    csv += [`"${u.hoten||""}"`,`"${u.mssv||""}"`,`"${u.email||""}"`,`"${u.sdt||""}"`,`"${u.trangThai||""}"`,`"${(u.suKienThamGia||[]).join(", ")}"`,`"${u.updatedAt ? new Date(u.updatedAt).toLocaleString("vi-VN") : ""}"`].join(";") + "\n";
  });
...
// ── 9. HOOK VÀO addParticipant & handleCheckin ────────
// FIX: Hook sau DOMContentLoaded khi module đã chạy xong
document.addEventListener("DOMContentLoaded", () => {
  injectDatabasePage();
  syncUserDatabase(); // Đồng bộ ban đầu
  ...
  if (typeof _origAdd === "function") {
    window.addParticipant = function() {
      ...
      setTimeout(syncUserDatabase, 500);
    };
  }
  ...
  if (typeof _origCheckin === "function") {
    window.handleCheckin = function(code) {
      ...
      setTimeout(() => syncCheckinStatus(mssv), 500);
    };
  }
});
