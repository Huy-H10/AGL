...
// FIX: window.go override moved to DOMContentLoaded (module runs first)
...
function renderBudgetPage() {
  ...
  let budgetRows = "";
  if (budgets.length === 0) {
    budgetRows = `<tr><td colspan="6" style="text-align:center;color:#aaa;padding:20px;">Chưa có ngân sách nào. Hãy tạo mới!</td></tr>`;
  } else {
    budgets.forEach((b, i) => {
      const trangThai = `<span style="background:#2ecc71;color:#fff;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:700;">${b.trangThai}</span>`;
    ...
    <div class="card" style="max-width:900px;">
      ...
      <div class="table-container">
        <table>
          <thead>
            <tr>
              <th>Sự kiện</th><th>Tổng ngân sách</th>
...
// -------------------------------------------------------
// 3. MỞ MODAL TẠO NGÂN SÁCH
// -------------------------------------------------------
function openBudgetModal() {
  if (!isAdminForBudget()) {
    if (typeof window.showToast === "function") window.showToast("Bạn không có quyền tạo ngân sách!", "error");
    return;
  }
...
function openEditBudgetModal(index) {
  if (!isAdminForBudget()) {
    if (typeof window.showToast === "function") window.showToast("Bạn không có quyền chỉnh sửa!", "error");
    return;
  }
  const budgets = getBudgets();
...
function validateBudgetForm() {
  ...
  if (!tenSuKien) { showBudgetError("budget_tenSuKien","budget_tenSuKienErr","Vui lòng chọn sự kiện!"); valid=false; }
  if (!tongNganSach || parseFloat(tongNganSach) <= 0) { showBudgetError("budget_tongNganSach","budget_tongNganSachErr","Vui lòng nhập ngân sách hợp lệ!"); valid=false; }
  if (!tuNgay) { showBudgetError("budget_tuNgay","budget_tuNgayErr","Vui lòng chọn ngày bắt đầu!"); valid=false; }
  if (!denNgay) { showBudgetError("budget_denNgay","budget_denNgayErr","Vui lòng chọn ngày kết thúc!"); valid=false; }
  if (tuNgay && denNgay && denNgay < tuNgay) { showBudgetError("budget_denNgay","budget_denNgayErr","Ngày kết thúc phải sau ngày bắt đầu!"); valid=false; }
  return valid;
}
...
function saveBudget() {
  if (!isAdminForBudget()) {
    if (typeof window.showToast === "function") window.showToast("Bạn không có quyền thực hiện!", "error");
    return;
  }
  ...
  if (index === "") {
    ...
    if (typeof window.showToast === "function") window.showToast("✅ Tạo ngân sách thành công!");
  } else {
    budgets[parseInt(index)] = newBudget;
    if (typeof window.showToast === "function") window.showToast("✅ Cập nhật ngân sách thành công!");
  }
  saveBudgets(budgets);
  closeBudgetModal();
  renderBudgetPage();
...
function deleteBudget(index) {
  if (!isAdminForBudget()) {
    if (typeof window.showToast === "function") window.showToast("Bạn không có quyền xóa!", "error");
    return;
  }
  if (confirm("Bạn có chắc muốn xóa ngân sách này?")) {
    ...
    saveBudgets(budgets);
    renderBudgetPage();
    if (typeof window.showToast === "function") window.showToast("🗑️ Đã xóa ngân sách!");
  }
}
    ...
    <div id="budgetModal" style="
      ...
      <div style="
        ...
        <div style="display:flex;gap:10px;margin-top:10px;">
          <button onclick="saveBudget()" style="flex:1;padding:13px;background:linear-gradient(135deg,#ff6a88,#e84393);color:#fff;border:none;border-radius:14px;font-weight:700;cursor:pointer;">
            💾 Lưu ngân sách
          </button>
          <button onclick="closeBudgetModal()" style="flex:1;padding:13px;border:2px solid #ff6a88;background:#fff;color:#ff6a88;border-radius:14px;font-weight:700;cursor:pointer;">
            ❌ Hủy
          </button>
        </div>
      </div>
...
// -------------------------------------------------------
// 11. HOOK window.go — ĐỢI MODULE CHẠY XONG
// FIX: dùng DOMContentLoaded thay vì ghi đè ngay khi load
// -------------------------------------------------------
document.addEventListener("DOMContentLoaded", function () {
  injectBudgetPage();

  // Module đã chạy xong khi DOMContentLoaded fires
  // Nên window.go đã được định nghĩa ở thời điểm này
